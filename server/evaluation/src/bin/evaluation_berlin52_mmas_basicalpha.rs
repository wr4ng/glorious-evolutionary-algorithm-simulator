...
fn main() {
	...
	let mut fitness_times = Vec::with_capacity(alphas.len());
	let mut optimal_finds = Vec::with_capacity(alphas.len());
...
").unwrap();
			let fit = mmas_tsp_optimize_rho(tsp, alpha, PheromoneUpdateStrategy::BestSoFar, &mut rng);
			for i in 0..fit.len() {
				fitnesses[i] += fit[i]
			}
	...
	let mut fitness_times_gb = Vec::with_capacity(alphas.len());
	let mut optimal_finds_gb = Vec::with_capacity(alphas.len());
...
").unwrap();
			let fit = mmas_tsp_optimize_rho(tsp, alpha, PheromoneUpdateStrategy::GenerationBest, &mut rng);
			for i in 0..fit.len() {
				fitnesses[i] += fit[i]
			}
			if fit[fit.len()-1] == 7542.0 {optimals += 1}
		}
		fitness_times_gb.push(fitnesses);
		optimal_finds_gb.push(optimals);
	}

	for (i, results) in fitness_times.into_iter().enumerate() {
        println!("MMAS Fitness based, BestSoFar (alpha = {}):", alphas[i]);
		println!("Found optimal solution {} times", optimal_finds[i]);
        print_results(results);
    }
	for (i, results) in fitness_times_gb.into_iter().enumerate() {
        println!("MMAS Fitness based, GenerationBest (alpha = {}):", alphas[i]);
		println!("Found optimal solution {} times", optimal_finds_gb[i]);
        print_results(results);
    }
...
fn mmas_tsp_optimize_rho<R: Rng>(tsp: TSP, alpha: f64, update_strat: PheromoneUpdateStrategy, rng: &mut R) -> Vec<f64> {
	let size = tsp.num_cities();
	let mut mmas = MMAStsp::new(tsp.distances(), tsp, size, 52, alpha, 4.0, 0.02, update_strat, true, 0.05, 1.0, rng);
