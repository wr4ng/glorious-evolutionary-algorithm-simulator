...
fn main() {
	...
	let qs = vec![1.0, 10.0, 50.0, 100.0, 500.0, 1000.0];

    let start = Instant::now();

	let mut fitness_times = Vec::with_capacity(qs.len());
	let mut optimal_finds = Vec::with_capacity(qs.len());
...
").unwrap();
			let fit = mmas_tsp_optimize_q(tsp, q, &mut rng);
			for i in 0..fit.len() {
				fitnesses[i] += fit[i]
			}
	...
	for (i, results) in fitness_times.into_iter().enumerate() {
        println!("MMAS Fitness based, BestSoFar (q = {}):", qs[i]);
...
fn mmas_tsp_optimize_q<R: Rng>(tsp: TSP, q: f64, rng: &mut R) -> Vec<f64> {
	...
	let mut mmas = MMAStsp::new(tsp.distances(), tsp, size, 52, 1.0, 4.0, 0.02, PheromoneUpdateStrategy::GenerationBest, true, 0.0, q, rng);
