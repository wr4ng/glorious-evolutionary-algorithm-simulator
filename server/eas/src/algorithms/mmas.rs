...
use crate::{
    fitness::FitnessFunction, rng::{MyRng}, search_space::{Bitstring, Permutation, SearchSpace}
};
    ...
    F: FitnessFunction<Permutation>
{
    ...
    fn decrease(&mut self){
        for i in 0..self.size{
            for ii in 0..self.size{
                self.pheromone[i][ii] = self.t_min.max(self.pheromone[i][ii] * (1.0 - self.evap_factor))
            }
        }
    }
    ...
    fn update(&mut self, paths: &Vec<Permutation>){
        for path in paths{
            ...
            if self.fitness_function.compare(fit_val, self.state.current_fitness) == std::cmp::Ordering::Greater{
